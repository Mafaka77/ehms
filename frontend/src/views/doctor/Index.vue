...
const handleEdit = (doctor) => {
  router.push(`/doctors/edit/${doctor._id}`)
}
...
const handleDelete = async (doctor) => {
  ...
  const res = await doctorStore.deleteDoctor(doctor._id)
  deletingId.value = null

  if (res.success) {
    snackbarStore.show({ message: res.message || 'Doctor deleted successfully!', type: 'success' })
    await loadDoctors()
  } else {
              ...
              <td class="px-6 py-4 text-center">
                <div class="flex items-center justify-center gap-2">
                  ...
                  <button 
                    ...
                    class="p-2 rounded-xl text-slate-400 hover:bg-rose-50 hover:text-rose-600 transition-all focus:outline-none focus:ring-2 focus:ring-rose-100 disabled:opacity-40"
                    title="Delete Doctor"
                  >
                    <svg v-if="deletingId === doctor._id" class="animate-spin w-4.5 h-4.5 text-rose-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                      <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                      <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    </svg>
