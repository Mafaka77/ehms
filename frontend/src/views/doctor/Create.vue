...
const loadData = async () => {
  ...
  try {
    ...
    console.error(err)
    snackbarStore.show({ message: 'Failed to load dropdown data.', type: 'error' })
  } finally {
...
const handleSubmit = async () => {
  ...
  try {
    const response = await doctorStore.createDoctor(form.value)
    if (response.success) {
      snackbarStore.show({ message: 'Doctor registered successfully!', type: 'success' })
      handleBack()
    } else {
      snackbarStore.show({ message: response.message || 'Failed to register doctor', type: 'error' })
    }
  } catch (err) {
    console.error(err)
...
<template>
  <div class="max-w-4xl mx-auto space-y-8 pb-10">
    ...
    <div class="flex items-center gap-4">
      ...
      <div>
        <h1 class="text-2xl font-bold text-slate-900">Add New Doctor</h1>
        <p class="text-slate-500 text-sm mt-0.5">Register a new clinical staff member.</p>
      </div>
    </div>
