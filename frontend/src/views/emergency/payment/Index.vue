...
const selectedDetailedVisit = ref(null)
...
const fetchVisits = async () => {
  try {
    ...
    if (pag) {
      ...
      totalItems.value = pag.total || emergencyStore.visits.length
    } else {
      totalPages.value = 1
      totalItems.value = emergencyStore.visits.length
    }
  } catch (error) {
    console.error('Error fetching emergency visits:', error)
    snackbarStore.show({
      message: emergencyStore.error || 'Failed to fetch emergency visits',
      type: 'error'
    })
  }
}
...
const fetchVisitDetails = async (visitId) => {
  try {
    selectedDetailedVisit.value = await emergencyStore.getVisitById(visitId)
  } catch (error) {
    console.error('Error fetching emergency visit details:', error)
    snackbarStore.show({
      message: error.response?.data?.message || 'Failed to load emergency visit details',
      type: 'error'
    })
  }
}
...
const handleSelectVisit = async (visit) => {
  ...
  await fetchVisitDetails(visit._id)
}

const handleBillGenerated = async (bill) => {
  // Re-fetch list to update statuses, and reload current selected visit details
  await fetchVisits()
  if (selectedVisit.value) {
    await fetchVisitDetails(selectedVisit.value._id)
  }
}
...
const submitPayment = async () => {
  ...
  try {
    await emergencyStore.processPayment(activeBill.value._id, paymentForm.value)
    snackbarStore.show({
...
<template>
  <div class="space-y-6 max-w-7xl mx-auto p-4 sm:p-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
      <div>
        <h1 class="text-2xl font-bold text-slate-900">Emergency Payments</h1>
        <p class="text-slate-500 mt-1 text-sm">Process bills and collect payments for emergency room visits.</p>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
      ...
      <div class="lg:col-span-7 bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden flex flex-col h-[700px]">
        ...
        <div class="p-6 border-b border-slate-100 bg-slate-50/30 space-y-4">
          ...
          <div class="relative w-full">
            ...
            <input 
              ...
              class="w-full pl-10 pr-4 py-2 bg-white border border-slate-200 rounded-xl text-sm placeholder-slate-400 text-slate-700 focus:outline-none focus:ring-2 focus:ring-rose-100 focus:border-rose-500 transition-all shadow-sm"
            />
          </div>
        </div>
        ...
        <div class="flex-grow overflow-y-auto">
          <!-- Loading state -->
          <div v-if="emergencyStore.loading && emergencyStore.visits.length === 0" class="flex flex-col items-center justify-center h-full text-slate-400 py-12">
            <svg class="animate-spin h-10 w-10 text-rose-600 mb-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
              <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
              <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
            <span class="text-sm font-medium">Loading emergency visits...</span>
          </div>

          <!-- Empty state -->
          <div v-else-if="emergencyStore.visits.length === 0" class="flex flex-col items-center justify-center h-full text-slate-400 p-8 py-16">
            ...
            <p class="text-slate-700 font-semibold text-base">No visits found</p>
            <p class="text-slate-400 text-xs mt-1 text-center max-w-xs">
              {{ searchQuery ? "No results match your search query." : "There are currently no visits with this payment status." }}
            </p>
          </div>
          ...
          <table v-else class="w-full text-left text-xs whitespace-nowrap">
            ...
            <tbody class="divide-y divide-slate-100 text-slate-700">
              <tr 
                ...
                :key="v._id"
                @click="handleSelectVisit(v)"
                :class="['hover:bg-slate-50/80 cursor-pointer transition-all', selectedVisit && selectedVisit._id === v._id ? 'bg-rose-50/50 hover:bg-rose-50 font-medium' : '']"
              >
                <td class="px-6 py-4">
                  <span class="font-mono font-bold text-slate-900">{{ v.visitNo }}</span>
                </td>
                <td class="px-6 py-4 text-slate-500">
                  {{ formatDate(v.arrivalDateTime) }}
                </td>
                <td class="px-6 py-4">
                ...
                <td class="px-6 py-4 text-right font-mono font-bold text-slate-900">
                  {{ formatCurrency(v.consultationFee) }}
                </td>
                <td class="px-6 py-4 text-center">
                  <span :class="['px-2 py-0.5 text-[10px] font-bold rounded uppercase border', getStatusColor(v.paymentStatus)]">
      ...
      <div class="lg:col-span-5 h-[700px]">
        <EmergencyPaymentView 
          v-if="selectedVisit && selectedDetailedVisit" 
          :visit="selectedDetailedVisit" 
        ...
        <div v-else class="bg-white border border-slate-100 rounded-2xl shadow-sm h-full flex flex-col justify-center items-center text-slate-400 p-8 text-center">
          ...
          <p class="text-slate-700 font-semibold text-base">No Visit Selected</p>
          <p class="text-slate-400 text-xs mt-1 max-w-[240px]">
            Please click on any Emergency visit from the list on the left to see details and process billing.
          </p>
        </div>
      </div>
    </div>
    ...
    <div v-if="showPaymentModal && activeBill" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div class="p-6 space-y-4">
          <!-- Totals Info -->
          <div class="bg-rose-50/40 border border-rose-100 p-4 rounded-xl grid grid-cols-2 gap-3 text-xs">
            <div>
            ...
            <div class="col-span-2 border-t border-rose-100 pt-2 flex justify-between items-center text-sm">
              <span class="font-semibold text-rose-950">Amount Payable:</span>
          ...
          <div class="space-y-4">
            ...
            <div class="space-y-1.5">
              ...
              <select 
                ...
                class="w-full px-3.5 py-2.5 bg-white border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-rose-100 focus:border-rose-500 text-slate-700 transition-all shadow-sm"
              >
            ...
            <div class="space-y-1.5">
              ...
              <textarea 
                ...
                rows="2"
                class="w-full px-3.5 py-2.5 bg-white border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-rose-100 focus:border-rose-500 text-slate-700 transition-all shadow-sm resize-none"
              ></textarea>
            </div>
          </div>
        </div>
