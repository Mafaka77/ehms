...
const printInvoicePDF = async () => {
  ...
  try {
    const element = document.querySelector('.emergency-print-receipt-container')
  ...
  <div v-if="show && billDetails" class="fixed inset-0 z-50 flex items-center justify-center p-4 screen-only animate-in fade-in duration-200">
    ...
    <div class="relative bg-slate-100 rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
      ...
      <div class="p-6 overflow-y-auto flex justify-center bg-slate-100 flex-grow">
        <!-- Receipt Sheet (210mm x 148mm ratio) -->
        <div class="emergency-print-receipt-container select-none">
          <div class="receipt-content">
            ...
            <div class="receipt-header">
              ...
              <p>Mob: +91 8974326872 | Email: emmanuelhospital4@gmail.com</p>
              <hr class="receipt-divider" />
              <h2>EMERGENCY CONSULTATION BILL / INVOICE</h2>
            </div>
            ...
            <div class="demographics">
              <div>
                ...
                <p><strong>Visit No:</strong> <span class="font-mono">{{ visit.visitNo }}</span></p>
              </div>
      ...
      <div class="p-4 bg-white border-t border-slate-100 flex justify-end gap-3 screen-only">
        <button
          @click="emit('close')"
          class="px-4 py-2 text-xs font-semibold text-slate-650 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors"
        >
          Close
        </button>
        <button
          ...
          class="px-5 py-2 text-xs font-bold text-white bg-rose-600 hover:bg-rose-700 rounded-xl shadow-lg shadow-rose-100 transition-all flex items-center gap-1.5 disabled:opacity-50"
        >
