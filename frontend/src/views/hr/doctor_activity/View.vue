...
const activeTab = ref('activity') // 'activity', 'ipd', 'opd'
    ...
    <div v-else-if="doctor" class="space-y-6">
      ...
      <div class="bg-white rounded-2xl border border-slate-100 p-6 shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        ...
        <div class="grid grid-cols-3 gap-3 w-full md:w-auto text-center">
          <div class="bg-sky-50 border border-sky-100 rounded-xl px-4 py-2.5">
            <p class="text-xs font-semibold text-sky-600 uppercase tracking-wider">OPD Today</p>
            <p class="text-xl font-black text-sky-700 mt-0.5">{{ metrics?.opdTodayCount || 0 }}</p>
          </div>
          <div class="bg-violet-50 border border-violet-100 rounded-xl px-4 py-2.5">
            <p class="text-xs font-semibold text-violet-600 uppercase tracking-wider">Active IPD</p>
            <p class="text-xl font-black text-violet-700 mt-0.5">{{ metrics?.activeIpdCount || 0 }}</p>
          </div>
          <div class="bg-indigo-50 border border-indigo-100 rounded-xl px-4 py-2.5">
            <p class="text-xs font-semibold text-indigo-600 uppercase tracking-wider">Total OPD</p>
      ...
      <div class="bg-white rounded-2xl border border-slate-100 shadow-xs overflow-hidden">
        <div class="flex items-center border-b border-slate-100 bg-slate-50/50 px-4">
          ...
          <button
            ...
            class="px-4 py-3 text-xs border-b-2 transition-all cursor-pointer flex items-center gap-2"
          >
