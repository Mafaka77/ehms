...
const handleSubmit = async () => {
  ...
  try {
    if (props.category) {
      // Edit Mode
      const response = await labStore.updateCategory(props.category._id, form)
      if (response.success) {
        snackbarStore.show({
      ...
      // Create Mode
      const response = await labStore.createCategory(form)
      if (response.success) {
        snackbarStore.show({
      ...
      <div v-if="show" class="fixed inset-0 z-[999] flex items-center justify-center p-4 overflow-x-hidden overflow-y-auto">
        ...
        <div class="bg-white rounded-2xl w-full max-w-lg shadow-2xl border border-slate-200 overflow-hidden transform transition-all relative z-10">
          ...
          <div class="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
            <div>
              <h3 class="text-lg font-bold text-slate-900">{{ category ? 'Edit Lab Category' : 'Add New Lab Category' }}</h3>
              <p class="text-xs text-slate-500 mt-0.5">{{ category ? 'Modify the details of this category.' : 'Define a new category for laboratory tests.' }}</p>
            </div>
            <button 
              @click="handleClose" 
