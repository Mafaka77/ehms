...
const fetchCategories = async () => {
  try {
    await labStore.fetchCategories(currentPage.value, limit.value, labStore.searchQuery);
  } catch (err) {
    console.error(err)
  }
}
...
const handleCategoryCreated = (newCat) => {
  if (currentPage.value === 1) {
    labStore.categories.unshift(newCat)
    if (labStore.categories.length > limit.value) {
      labStore.categories.pop()
    }
    labStore.pagination.total++
  } else {
    fetchCategories()
  }
}
...
const handleDelete = async (cat) => {
  if (confirm(`Are you sure you want to delete the ${cat.name} category?`)) {
    const response = await labStore.deleteCategory(cat._id)
    if (response.success) {
      snackbarStore.show({
        message: response.message,
        type: 'success'
      })
      if (labStore.categories.length === 0 && currentPage.value > 1) {
        currentPage.value--
      } else {
        fetchCategories()
      }
...
<template>
  <div class="space-y-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold text-slate-900">Lab Categories</h1>
        <p class="text-slate-500 mt-1 text-sm">Manage laboratory test categories.</p>
      </div>
      <button 
        @click="openAddModal"
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
      ...
      <div v-else-if="filteredCategories.length === 0" class="p-6 text-center text-slate-500 py-24">
        ...
        <p class="text-slate-400 text-sm mt-1 max-w-sm mx-auto">
          {{ labStore.searchQuery ? "No results match your search query. Try typing something else." : "Get started by adding the first lab category." }}
        </p>
        <button 
        ...
        <table class="w-full border-collapse text-left">
          <thead>
            <tr class="bg-slate-50 border-b border-slate-100">
              <th class="text-slate-500 font-semibold text-xs uppercase px-6 py-4.5 tracking-wider w-1/3">Name</th>
              <th class="text-slate-500 font-semibold text-xs uppercase px-6 py-4.5 tracking-wider w-1/3">Status</th>
          ...
          <tbody class="divide-y divide-slate-100">
            ...
            >
              ...
              <!-- Status -->
              <td class="px-6 py-4">
                <span v-if="cat.isActive" class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-emerald-100 text-emerald-800">
                  Active
                </span>
      ...
      <!-- Pagination Footer -->
      <div 
        v-if="labStore.pagination.total > 0" 
      ...
      >
        ...
        <span class="text-sm text-slate-500 font-medium">
          ...
          to 
          <span class="text-slate-800 font-semibold">{{ Math.min(currentPage * limit, labStore.pagination.total) }}</span> 
          of 
          <span class="text-slate-800 font-semibold">{{ labStore.pagination.total }}</span> 
          entries
        </span>

        <!-- Pagination Buttons -->
        <div v-if="labStore.pagination.pages > 1" class="flex items-center gap-2">
          <!-- Previous Button -->
          <button 
          ...
          <!-- Page Numbers -->
          <button 
            v-for="page in labStore.pagination.pages" 
            :key="page"
          ...
          <!-- Next Button -->
          <button 
            @click="currentPage < labStore.pagination.pages && currentPage++"
            :disabled="currentPage === labStore.pagination.pages"
            class="inline-flex items-center justify-center p-2 rounded-xl border border-slate-200 bg-white text-slate-600 hover:bg-slate-50 hover:text-slate-800 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
            title="Next Page"
          >
            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
