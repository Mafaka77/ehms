...
const fetchOrders = async () => {
  await labStore.fetchOrders(currentPage.value, limit.value, searchQuery.value)
}
...
const handleDelete = async (id) => {
  if (confirm('Are you sure you want to delete this order? This will also delete all associated order items.')) {
    const res = await labStore.deleteOrder(id)
    if (res.success) {
      snackbarStore.show({ message: 'Order deleted successfully', type: 'success' })
      if (labStore.orders.length === 0 && currentPage.value > 1) {
        currentPage.value--
      } else {
        fetchOrders()
      }
...
<template>
  <div class="space-y-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold text-slate-900">Lab Orders</h1>
        <p class="text-slate-500 mt-1 text-sm">View and manage patient laboratory orders.</p>
      </div>
      <button 
        @click="router.push({ name: 'laboratory-order-create' })"
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
      ...
      <!-- Search -->
      <div class="p-6 border-b border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-50/30">
        <h2 class="text-lg font-semibold text-slate-800">Order List</h2>
        <div class="relative w-full sm:w-80">
          ...
          <input 
            v-model="searchQuery"
            type="text" 
            placeholder="Search patient name, order no, code..." 
      ...
      <!-- Empty State -->
      <div v-else-if="labStore.orders.length === 0" class="p-6 text-center text-slate-500 py-24">
        ...
        <p class="text-slate-400 text-sm mt-1 max-w-sm mx-auto">
          {{ searchQuery ? "No results match your search query." : "Click the 'Create Order' button to register a new lab order." }}
        </p>
      </div>
        ...
        <table class="w-full border-collapse text-left whitespace-nowrap">
          <thead>
            <tr class="bg-slate-50 border-b border-slate-100">
              <th class="text-slate-500 font-semibold text-xs uppercase px-6 py-4.5 tracking-wider">Order No</th>
              <th class="text-slate-500 font-semibold text-xs uppercase px-6 py-4.5 tracking-wider">Date</th>
              <th class="text-slate-500 font-semibold text-xs uppercase px-6 py-4.5 tracking-wider">Patient & Doctor</th>
              <th class="text-slate-500 font-semibold text-xs uppercase px-6 py-4.5 tracking-wider">Priority</th>
              <th class="text-slate-500 font-semibold text-xs uppercase px-6 py-4.5 tracking-wider text-right">Amount</th>
          ...
          <tbody class="divide-y divide-slate-100">
            ...
            >
              <!-- Order No -->
              <td class="px-6 py-4">
                <span class="font-mono text-sm font-semibold text-slate-800">{{ order.orderNo }}</span>
              </td>
              <!-- Date -->
              <td class="px-6 py-4">
                <div class="flex flex-col text-sm">
                  <span class="text-slate-700">{{ new Date(order.orderDate).toLocaleDateString() }}</span>
                  <span class="text-slate-400 text-xs">{{ new Date(order.orderDate).toLocaleTimeString() }}</span>
                </div>
              </td>
              <!-- Patient & Doctor -->
              <td class="px-6 py-4">
                <div class="flex flex-col">
                  <span class="text-sm font-medium text-slate-800">
                    Patient: <span class="text-slate-600">{{ order.patientId?.fullName || order.patientId }}</span>
                  </span>
                  <span v-if="order.referral === 'Self'" class="text-xs text-slate-500 mt-0.5">Self-Referred</span>
                  <span v-else class="text-xs text-slate-500 mt-0.5">
                    Doctor: <span class="font-mono text-xs">{{ order.doctorId?.fullName || order.doctorId || 'N/A' }}</span>
                  </span>
                </div>
              </td>
              <!-- Priority -->
              ...
              <td class="px-6 py-4 text-right">
                <span class="font-medium text-sm text-slate-700">₹{{ order.totalAmount?.toLocaleString() || '0' }}</span>
              </td>
              <!-- Status -->
              <td class="px-6 py-4 text-center">
                <span 
                  class="text-[10px] uppercase font-bold tracking-wider px-2 py-1 rounded border"
              ...
              <td class="px-6 py-4 text-center">
                <div class="flex items-center justify-center gap-2">
                  <button 
                    @click="router.push({ name: 'laboratory-order-edit', params: { id: order._id } })"
