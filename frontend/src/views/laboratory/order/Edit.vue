...
const fetchOrderData = async () => {
  ...
  try {
    loading.value = true
    const orderData = await labStore.getOrderById(orderId)
    
    // Populate form
    if (orderData.patientId && typeof orderData.patientId === 'object') {
      form.patientId = orderData.patientId._id
      selectedPatient.value = orderData.patientId
      patientSearchQuery.value = orderData.patientId.fullName
    } else {
      form.patientId = orderData.patientId
      patientSearchQuery.value = orderData.patientId
    }
    
    if (orderData.doctorId && typeof orderData.doctorId === 'object') {
      form.doctorId = orderData.doctorId._id
      selectedDoctor.value = orderData.doctorId
      doctorSearchQuery.value = orderData.doctorId.fullName
    } else if (orderData.doctorId) {
      form.doctorId = orderData.doctorId
      doctorSearchQuery.value = orderData.doctorId
    } else {
      form.doctorId = ''
    }
    
    form.referral = orderData.referral || 'Doctor'
    form.priority = orderData.priority
    form.status = orderData.status || 'ORDERED'
    form.clinicalNotes = orderData.clinicalNotes || ''
    form.remarks = orderData.remarks || ''
    
    // Populate selected tests
    if (orderData.items && orderData.items.length > 0) {
      selectedTests.value = orderData.items.map(item => ({
        testId: item.testId,
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const response = await labStore.updateOrder(orderId, orderData)
  ...
  <div class="max-w-7xl mx-auto space-y-8">
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-4">
        ...
        <div>
          <h1 class="text-2xl font-bold text-slate-900">Edit Lab Order</h1>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div v-if="form.referral === 'Doctor'" class="relative">
                ...
                <div v-if="selectedDoctor" class="flex items-center justify-between px-4 py-2.5 bg-indigo-50 border border-indigo-200 rounded-xl">
                  <div class="flex flex-col">
                    <span class="text-sm font-bold text-indigo-900">{{ selectedDoctor.fullName || 'Doctor' }}</span>
                    <span class="text-xs text-indigo-700">{{ selectedDoctor.doctorCode || selectedDoctor.specializationId?.name || selectedDoctor._id }}</span>
                  </div>
