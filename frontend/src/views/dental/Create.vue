...
const selectedDoctorFee = computed(() => {
  ...
  const doctor = dentalStore.dentalDoctors.find(d => d._id === appointmentData.value.doctorId)
  return doctor ? doctor.consultationFee : null
})

onMounted(async () => {
  // Load doctors who have an OPD Remuneration rule
  await dentalStore.fetchDentalDoctors()
})
...
const submitAppointment = async () => {
  ...
  if (res.success) {
    ...
    router.push({ name: 'dental-appointment' })
  } else {
    snackbarStore.show({ message: res.message, type: 'error' })
  }
}
...
<template>
  <div class="max-w-4xl mx-auto space-y-8">
    ...
    <div class="flex items-center gap-4">
      ...
      <div>
        <h1 class="text-2xl font-bold text-slate-900">Dental Appointment</h1>
        <p class="text-slate-500 mt-1 text-sm">Create a new Dental Department appointment.</p>
      </div>
    </div>
    ...
    <div v-if="showCardModal && createdAppointment" class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 print:p-0 print:items-start print:justify-start">
      ...
      <div class="relative bg-slate-100 rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden print:max-h-none print:overflow-visible print:bg-white print:shadow-none print:rounded-none">
        ...
        <div class="flex items-center justify-between p-4 border-b border-slate-200 bg-white print:hidden">
          <div>
            <h2 class="text-lg font-bold text-slate-800">Appointment Booked Successfully!</h2>
            <p class="text-sm text-slate-500">Please print the Dental Card for the patient.</p>
          </div>
          <div class="flex items-center gap-3">
            <button 
              @click="closeModal"
