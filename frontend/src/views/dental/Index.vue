...
const fetchAppointments = async () => {
  loading.value = true
  const res = await dentalStore.fetchAppointments(filters.value)
  if (!res.success) {
...
onMounted(async () => {
  dentalStore.fetchDentalDoctors() // Non-blocking preload
  await fetchAppointments()
})

const handleDelete = async (id) => {
  if (confirm('Are you sure you want to delete this appointment? This action cannot be undone.')) {
    const res = await dentalStore.deleteAppointment(id);
    if (res.success) {
...
const handleGenerateReport = async () => {
  ...
  try {
    const res = await dentalStore.fetchAppointmentsReport(reportFilters.value)
...
const exportToExcel = (reportData) => {
  ...
  link.setAttribute("download", `Dental_Appointments_Report_${new Date().toISOString().split('T')[0]}.csv`)
...
const handleUpdate = async () => {
  ...
  const res = await dentalStore.updateAppointment(editForm.value._id, {
...
watch(() => editForm.value.doctorId, (newDocId) => {
  ...
  const doctor = dentalStore.dentalDoctors.find(d => d._id === newDocId)
  if (doctor) {
    editForm.value.consultationFee = doctor.consultationFee || 0
  }
})
</script>
  ...
  <div class="space-y-6 max-w-7xl mx-auto">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
      <div class="flex items-center gap-3.5">
        <div class="w-12 h-12 rounded-2xl bg-teal-50 border border-teal-100 flex items-center justify-center text-teal-600 shadow-2xs">
        ...
        <div>
          <div class="flex items-center gap-2">
            <h1 class="text-xl font-bold text-slate-900 tracking-tight">Dental Appointments</h1>
            <span class="px-2.5 py-0.5 text-xs font-extrabold bg-teal-50 text-teal-700 rounded-full border border-teal-100">
              {{ dentalStore.pagination.total }} Total
            </span>
          </div>
          <p class="text-xs text-slate-500 mt-0.5">Manage dental consultations, patient files, and treatment schedules.</p>
        </div>
      </div>
      ...
      <div class="flex items-center gap-2.5 w-full sm:w-auto">
        ...
        <button 
          @click="router.push({ name: 'dental-create' })"
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
      ...
      <div class="p-4 border-b border-slate-100 bg-slate-50/50 flex flex-col md:flex-row justify-between items-center gap-4">
        <div class="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          <div class="relative">
            ...
            >
              ...
              <option v-for="doc in dentalStore.dentalDoctors" :key="doc._id" :value="doc._id">
                {{ doc.fullName }}
              </option>
            </select>
          </div>
          ...
          <div class="relative min-w-[220px]">
            <input 
              ...
              placeholder="Search Appt ID, Patient..." 
              class="w-full pl-8 pr-3 py-2 bg-white border border-slate-200 rounded-xl text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 text-slate-700 shadow-2xs transition-all"
            />
          ...
          <button
            ...
            class="px-3.5 py-2 bg-indigo-50 text-indigo-700 border border-indigo-100 hover:bg-indigo-100 rounded-xl text-xs font-bold transition-all cursor-pointer shadow-2xs"
          >
            Search
          </button>
          ...
          <!-- Clear Filters -->
          <button 
            v-if="filters.status || filters.doctorId || filters.date || filters.search"
            @click="clearFilters"
        ...
        <div class="text-xs font-bold text-slate-500">
          Showing <span class="text-slate-800 font-extrabold">{{ dentalStore.appointments.length }}</span> of <span class="text-slate-800 font-extrabold">{{ dentalStore.pagination.total }}</span> appointments
        </div>
      </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left text-sm border-collapse">
          ...
          <tbody class="divide-y divide-slate-100 text-slate-700">
            ...
            <tr v-else-if="dentalStore.appointments.length === 0">
            ...
            <tr 
              v-else
              v-for="app in dentalStore.appointments" 
              :key="app._id"
              class="hover:bg-slate-50/60 transition-colors"
            >
              <td class="px-6 py-4">
                <span class="font-mono text-indigo-600 font-bold bg-indigo-50/70 border border-indigo-100/80 px-2 py-0.5 rounded-md text-xs inline-block">
                  {{ app.appointmentId }}
                </span>
              </td>
              <td class="px-6 py-4">
                <div class="flex items-center gap-3">
                  <div class="w-8 h-8 rounded-full bg-teal-50 text-teal-700 font-bold text-xs flex items-center justify-center shrink-0 border border-teal-100">
                    {{ app.patientId?.fullName?.charAt(0) || 'P' }}
                  </div>
                  <div>
                    <button 
                      v-if="app.paymentStatus === 'Paid'"
                      @click="router.push({ name: 'dental-view', params: { id: app._id } })"
                      class="font-bold text-slate-800 hover:text-indigo-600 hover:underline block leading-tight text-xs text-left cursor-pointer"
                    >
                      {{ app.patientId?.fullName || 'N/A' }}
                    </button>
                    <span v-else class="font-bold text-slate-800 block leading-tight text-xs">
                      {{ app.patientId?.fullName || 'N/A' }}
                    </span>
              ...
              <td class="px-6 py-4 text-center">
                <div class="flex items-center justify-center gap-1.5">
                  ...
                  <button 
                    v-if="app.paymentStatus === 'Paid'"
                    @click.stop="router.push({ name: 'dental-view', params: { id: app._id } })"
      ...
      <!-- Pagination Controls -->
      <div v-if="dentalStore.pagination.pages > 1" class="p-4 border-t border-slate-100 flex items-center justify-between bg-slate-50/50">
        <button 
          @click="filters.page--" 
        ...
        <span class="text-xs font-bold text-slate-600">
          Page {{ filters.page }} of {{ dentalStore.pagination.pages }}
        </span>
        <button 
          @click="filters.page++" 
          :disabled="filters.page === dentalStore.pagination.pages"
          class="px-4 py-2 text-xs font-bold text-slate-700 bg-white border border-slate-200 hover:border-slate-300 rounded-xl hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-2xs cursor-pointer"
        >
          Next
        </button>
    ...
    <div v-if="showCardModal && selectedAppointmentForPrint" class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      <div class="relative bg-slate-100 rounded-2xl shadow-2xl w-full max-w-5xl h-[90vh] flex flex-col overflow-hidden animate-in fade-in duration-200">
        ...
        <div class="flex items-center justify-between p-4 border-b border-slate-200 bg-white">
          <div>
            <h2 class="text-base font-bold text-slate-800">Dental Card Preview</h2>
            <p class="text-xs text-slate-500">Preview and print the Dental Card for this appointment.</p>
          </div>
          <div class="flex items-center gap-3">
            <button 
              @click="closeModal"
    ...
    <div v-if="showReportModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col animate-in fade-in duration-200">
        ...
        <div class="flex items-center justify-between p-6 border-b border-slate-100">
          <div>
            <h2 class="text-base font-bold text-slate-800">Generate Dental Report</h2>
        ...
        <div class="p-6 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
          ...
          >
            ...
            <svg v-else class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 01-2-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
            Export to Excel
          </button>
        </div>
      </div>
    ...
    <div v-if="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col animate-in fade-in duration-200">
        ...
        <div class="flex items-center justify-between p-6 border-b border-slate-100">
          <div>
            <h2 class="text-base font-bold text-slate-800">Edit Dental Appointment</h2>
            <p class="text-xs text-slate-500 mt-0.5">Update details of the dental appointment.</p>
          </div>
          <button 
        ...
        <div class="p-6 space-y-4">
          ...
          <div v-if="editForm.hasBill" class="p-3 bg-amber-50 border border-amber-200/80 rounded-xl flex gap-2.5 text-xs text-amber-800">
            ...
            <div>
              ...
              <p class="mt-0.5">A bill is already generated for this appointment. Modifying the doctor or consultation fee is disabled.</p>
            </div>
          </div>
