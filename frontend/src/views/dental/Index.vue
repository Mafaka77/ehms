...
const syncAppointmentDates = async () => {
  if (confirm('Are you sure you want to sync all appointment dates with their creation dates?')) {
    ...
    const res = await dentalStore.syncAppointmentDates()
...
onMounted(async () => {
  await dentalStore.fetchDentalDoctors() // Preload doctors for filter
...
<template>
  <div class="space-y-6 max-w-7xl mx-auto">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
      <div>
        <h1 class="text-2xl font-bold text-slate-900">Dental Appointments</h1>
      ...
      <div class="flex items-center gap-3 w-full sm:w-auto">
        ...
        <button 
          ...
          class="bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 px-5 py-2.5 rounded-xl font-bold text-sm shadow-sm transition-all flex items-center gap-2 w-full sm:w-auto justify-center cursor-pointer"
        >
          <svg class="w-5 h-5 text-slate-500" :class="{ 'animate-spin': isSyncing }" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
          {{ isSyncing ? 'Syncing...' : 'Sync Appointment Date' }}
        </button>
        <button 
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
      ...
      <div class="p-4 border-b border-slate-100 bg-slate-50/50 flex flex-col md:flex-row justify-between items-center gap-4">
        <div class="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          <div class="relative min-w-[240px]">
            <input 
              ...
              class="w-full pl-9 pr-3 py-2 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 text-slate-700"
            />
          ...
          <button
            ...
            class="px-4 py-2 bg-indigo-50 text-indigo-700 border border-indigo-200 hover:bg-indigo-100 rounded-lg text-sm font-semibold transition-colors"
          >
            Search
          </button>
        ...
        <div class="text-sm font-semibold text-slate-600 whitespace-nowrap">
          Total: {{ dentalStore.pagination.total }}
        </div>
      </div>
      ...
      <!-- Pagination -->
      <div v-if="dentalStore.pagination.pages > 1" class="p-4 border-t border-slate-100 flex items-center justify-between bg-slate-50/30">
        <button 
          @click="filters.page--" 
    ...
    <div v-if="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col animate-fade-in">
        ...
        <div class="flex items-center justify-between p-6 border-b border-slate-100">
          <div>
            <h2 class="text-lg font-bold text-slate-800">Edit Dental Appointment</h2>
