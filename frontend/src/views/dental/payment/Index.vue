...
import DentalPaymentView from './View.vue'
...
const fetchAppointments = async () => {
  try {
    await dentalStore.fetchAppointments({
    ...
    const pag = dentalStore.pagination
    if (pag) {
      totalPages.value = pag.pages || 1
      totalItems.value = pag.total || dentalStore.appointments.length
    } else {
      totalPages.value = 1
      totalItems.value = dentalStore.appointments.length
    }
  } catch (error) {
    console.error('Error fetching appointments:', error)
    snackbarStore.show({
      message: dentalStore.error || 'Failed to fetch appointments',
      type: 'error'
    })
  }
}
...
const fetchAppointmentDetails = async (apptId) => {
  try {
    selectedDetailedAppointment.value = await dentalStore.getAppointmentById(apptId)
  } catch (error) {
...
const submitPayment = async () => {
  ...
  try {
    await dentalStore.processPayment(activeBill.value._id, paymentForm.value)
    snackbarStore.show({
...
<template>
  <div class="space-y-6 max-w-7xl mx-auto p-4 sm:p-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
      <div>
        <h1 class="text-2xl font-bold text-slate-900">Dental Payments</h1>
        <p class="text-slate-500 mt-1 text-sm">Process bills and collect payments for Dental appointments.</p>
      </div>
    </div>
      ...
      <div class="lg:col-span-7 bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden flex flex-col h-[700px]">
        ...
        <div class="p-6 border-b border-slate-100 bg-slate-50/30 space-y-4">
          <div class="flex flex-col sm:flex-row gap-3 items-center justify-between">
            <h2 class="text-lg font-semibold text-slate-800 self-start">Dental Appointments</h2>
        ...
        <div class="flex-grow overflow-y-auto">
          <!-- Loading state -->
          <div v-if="dentalStore.loading && dentalStore.appointments.length === 0" class="flex flex-col items-center justify-center h-full text-slate-400 py-12">
          ...
          <!-- Empty state -->
          <div v-else-if="dentalStore.appointments.length === 0" class="flex flex-col items-center justify-center h-full text-slate-400 p-8 py-16">
          ...
          <table v-else class="w-full text-left text-xs whitespace-nowrap">
            ...
            <tbody class="divide-y divide-slate-100 text-slate-700">
              <tr 
                v-for="appt in dentalStore.appointments" 
                :key="appt._id"
      ...
      <div class="lg:col-span-5 h-[700px]">
        ...
        <div v-else class="bg-white border border-slate-100 rounded-2xl shadow-sm h-full flex flex-col justify-center items-center text-slate-400 p-8 text-center">
          ...
          <p class="text-slate-400 text-xs mt-1 max-w-[240px]">
            Please click on any Dental appointment from the list on the left to see details and process billing.
          </p>
        </div>
      </div>
    </div>
