...
const openAddModal = async () => {
  const dentalCat = chargeCategories.value.find(c => c.code && c.code.toUpperCase() === 'DENTAL') 
    || chargeCategories.value.find(c => c.name && c.name.toUpperCase().includes('DENTAL')) 
    || (chargeCategories.value.length > 0 ? chargeCategories.value[0] : null);

  chargeForm.value = {
    chargeCategoryId: dentalCat ? dentalCat._id : '',
    chargeMasterId: '',
    doctorId: '',
    description: '',
    rate: 0,
...
const submitCharge = async () => {
  ...
  const res = await dentalStore.addCharge(props.appointment._id, payload)
  if (res.success) {
...
const saveCharge = async (charge) => {
  ...
  loading.value = true
  const res = await dentalStore.updatePatientCharge(props.appointment._id, charge._id, {
  ...
  <div class="space-y-4">
    ...
    <div v-else-if="charges.length === 0" class="border border-dashed border-slate-200 rounded-2xl p-12 text-center space-y-3 bg-slate-50/50">
      ...
      <div>
        ...
        <p class="text-xs text-slate-400 mt-1 max-w-sm mx-auto">No dental charges have been billed or generated for this patient yet.</p>
      </div>
      <button 
        @click="openAddModal" 
