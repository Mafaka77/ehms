...
const loadMasterData = async () => {
  try {
    ...
    form.value.code = await nursingStore.generateStationCode()
  } catch (err) {
    form.value.code = ''
    console.error('Failed to load master data or generate station code', err)
  }
}
...
const handleFormSubmit = async () => {
  ...
  try {
    const response = await nursingStore.createNursingStation(form.value)
    if (response.success) {
      snackbarStore.show({
...
<template>
  <div class="space-y-8 pb-12 max-w-4xl mx-auto">
    ...
    <div class="flex items-center justify-between border-b border-slate-100 pb-5">
      <div>
        <h1 class="text-2xl font-bold text-slate-900">Add Nursing Station</h1>
        <p class="text-slate-500 mt-1 text-sm">Create a new nursing station and configure physical bed monitoring rules.</p>
      </div>
      <button 
