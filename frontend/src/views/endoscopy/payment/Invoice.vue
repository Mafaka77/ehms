...
const generateInvoicePDF = async () => {
  ...
  try {
    ...
    const filename = `Endoscopy_Invoice_${patientName}.pdf`
      ...
      <div class="flex-grow flex flex-col relative bg-slate-600 min-h-[500px]">
        ...
        <div v-if="printingPDF" class="absolute inset-0 flex items-center justify-center bg-slate-800 z-50">
          <div class="flex flex-col items-center">
            <span class="animate-spin rounded-full h-10 w-10 border-4 border-teal-400 border-t-transparent mb-3"></span>
        ...
        <div v-show="!pdfPreviewUrl" class="absolute inset-0 overflow-y-auto bg-slate-200 p-8 flex justify-center z-0">
          <div ref="receiptRef" class="opd-print-receipt-container select-none bg-white">
            ...
            <table class="items-table">
              ...
              <tbody>
                <tr v-for="item in billDetails.items" :key="item._id || item.description">
                  ...
                  <td class="text-right font-mono font-semibold">{{ formatCurrency(item.netAmount || item.amount) }}</td>
                </tr>
              </tbody>
            </table>
...
.receipt-header h1 { font-size: 14px; font-weight: 900; text-transform: uppercase; letter-spacing: -0.025em; color: #0f172a; margin: 0 0 4px 0; }
.receipt-header p { font-size: 10px; color: #64748b; margin: 0; }
.receipt-divider { border: 0; border-top: 1px dashed #cbd5e1; margin: 8px 0; }
.receipt-header h2 { font-size: 11px; font-weight: bold; letter-spacing: 0.05em; color: #1e293b; margin: 4px 0 0 0; }
.demographics { display: grid; grid-template-columns: 1fr 1fr; column-gap: 16px; row-gap: 2px; font-size: 10px; margin-bottom: 8px; line-height: 1.25; }
.demographics p { margin: 0; color: #334155; }
.demographics .patient-name { color: #0f172a; font-weight: bold; }
.doctor-section { border-top: 1px dashed #e2e8f0; padding-top: 5px; font-size: 10px; margin-bottom: 10px; color: #334155; }
.doctor-section p { margin: 0 0 2px 0; }
.items-table { width: 100%; border-collapse: collapse; font-size: 10px; margin-bottom: 12px; }
.items-table th { border-top: 1px dashed #cbd5e1; border-bottom: 1px dashed #cbd5e1; font-weight: bold; padding: 5px 0; color: #475569; }
.items-table td { border-bottom: 1px dashed #f1f5f9; padding: 5px 0; color: #334155; }
.discount-col { color: #16a34a !important; }
.financials-summary { width: 50%; margin-left: auto; font-size: 10px; border-top: 1px dashed #cbd5e1; padding-top: 8px; line-height: 1.25; }
.financials-summary > div { margin-bottom: 3px; }
.financials-summary .net-payable { border-top: 1px dashed #e2e8f0; padding-top: 4px; color: #064e3b; font-size: 11px; font-weight: bold; }
.financials-summary .balance-due { border-top: 1px dotted #e2e8f0; padding-top: 4px; color: #dc2626; font-weight: bold; }
.signatures { margin-top: 16px; display: flex; justify-content: space-between; font-size: 8px; color: #64748b; line-height: 1.25; }
.signatures .operator-name { font-weight: bold; color: #1e293b; margin: 2px 0 0 0; }
.signatures .sig-line { border-top: 1px dotted #cbd5e1; padding-top: 4px; display: inline-block; }
.notice { text-align: center; margin-top: 12px; font-size: 8px; color: #94a3b8; font-style: italic; line-height: 1.25; }
.notice p { margin: 0; }
.notice .wish { font-weight: bold; color: #475569; margin-top: 2px; }

@media print {
  @page { size: A5 landscape; margin: 0; }
  body * { visibility: hidden; }
  .opd-print-receipt-container, .opd-print-receipt-container * { visibility: visible; }
  .opd-print-receipt-container { position: absolute !important; left: 0 !important; top: 0 !important; width: 210mm !important; height: 148mm !important; overflow: hidden !important; height: auto !important; margin: 0 !important; padding: 10mm !important; box-shadow: none !important; border: none !important; z-index: 999999 !important; background-color: white !important; }
}
</style>
