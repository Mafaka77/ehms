...
import EndoscopyPaymentView from './View.vue'
...
const paymentStatusFilter = ref('UNPAID')
...
const fetchOrders = async () => {
  try {
    ...
    await endoscopyStore.fetchOrders(
      currentPage.value,
      limit.value,
      searchQuery.value,
    ...
    const pag = endoscopyStore.orderPagination
    if (pag) {
      totalPages.value = pag.pages || 1
      totalItems.value = pag.total || endoscopyStore.orders.length
    } else {
      totalPages.value = 1
      totalItems.value = endoscopyStore.orders.length
    }
  } catch (error) {
    console.error('Error fetching endoscopy orders:', error)
    snackbarStore.show({
      message: endoscopyStore.error || 'Failed to fetch endoscopy orders',
      type: 'error'
    })
  }
}
...
const fetchOrderDetails = async (orderId) => {
  try {
    const detailed = await endoscopyStore.getOrderById(orderId)
...
const submitPayment = async () => {
  ...
  try {
    await endoscopyStore.processPayment(activeBill.value._id, paymentForm.value)
    snackbarStore.show({
...
const handleExportPdf = async () => {
  ...
  try {
    ...
    await endoscopyStore.fetchOrders(1, 0, searchQuery.value, paymentStatusFilter.value, additionalParams)
    const exportOrders = endoscopyStore.orders || []

    if (!exportOrders || exportOrders.length === 0) {
      snackbarStore.show({ message: 'No endoscopy orders found for the selected criteria to export.', type: 'warning' })
    ...
    const printContent = `
      ...
      <html>
        <head>
          <title>Endoscopy Revenue & Payment Report - ${new Date().toLocaleDateString('en-IN')}</title>
          <style>
            ...
            .status-ipd { background: #ccfbf1; color: #0f766e; }
            ...
            .summary-table .value { font-weight: 800; color: #0f766e; text-align: right; font-size: 13px; font-family: monospace; }
        ...
        <body>
          <div class="header">
            <h1>Endoscopy Department Billing & Revenue Report</h1>
            <p>Hospital Endoscopy Orders Register</p>
          </div>
          ...
          <div class="stats-grid">
            ...
            <div class="stat-card">
              ...
              <div class="stat-value" style="color: #0f766e;">${statusBreakdown.PAID || 0} / ${statusBreakdown.UNPAID || 0}</div>
            </div>
          </div>
          ...
          <table class="summary-table">
            <tr>
              <td class="label">Grand Total Endoscopy Fees (${exportOrders.length} Orders):</td>
              <td class="value">₹${totalRevenue.toFixed(2)}</td>
            </tr>
          </table>

          <div class="footer">
            <div>Report generated automatically by EHMS Endoscopy Management Module.</div>
            <div class="sig-box">
              <div class="sig-line">Endoscopy In-Charge / Cashier</div>
            </div>
          </div>
    ...
    console.error(err)
    snackbarStore.show({ message: 'Failed to export endoscopy payments PDF report', type: 'error' })
  } finally {
    isExportingPdf.value = false
  }
}
...
<template>
  <div class="space-y-6 max-w-7xl mx-auto p-4 sm:p-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
      <div>
        <h1 class="text-2xl font-bold text-slate-900">Endoscopy Payments</h1>
        <p class="text-slate-500 mt-1 text-sm">Process bills and collect payments for endoscopy orders.</p>
      </div>
    </div>
      ...
      <div class="lg:col-span-7 bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden flex flex-col h-[700px]">
        ...
        <div class="p-6 border-b border-slate-100 bg-slate-50/30 space-y-4">
          <div class="flex flex-col sm:flex-row gap-3 items-center justify-between">
            <h2 class="text-lg font-semibold text-slate-800 self-start">Endoscopy Orders</h2>
            ...
            <div class="flex items-center gap-2">
              ...
              <button 
                ...
                class="px-3.5 py-1.5 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 hover:text-teal-700 font-bold text-xs rounded-lg shadow-sm transition-all flex items-center gap-1.5 shrink-0 cursor-pointer disabled:opacity-50"
              >
              ...
              <div class="flex bg-slate-100 p-0.5 rounded-lg text-xs font-semibold w-full sm:w-auto">
                <button 
                  ...
                  :class="['px-3 py-1.5 rounded-md transition-all flex-1 sm:flex-none text-center cursor-pointer', paymentStatusFilter === 'UNPAID' ? 'bg-white text-teal-600 shadow-sm' : 'text-slate-600 hover:text-slate-900']"
                >
                  Pending
                </button>
                <button 
                  @click="paymentStatusFilter = 'PAID'"
                  :class="['px-3 py-1.5 rounded-md transition-all flex-1 sm:flex-none text-center cursor-pointer', paymentStatusFilter === 'PAID' ? 'bg-white text-teal-600 shadow-sm' : 'text-slate-600 hover:text-slate-900']"
                >
                  Paid
                </button>
                <button 
                  @click="paymentStatusFilter = 'IPD'"
                  :class="['px-3 py-1.5 rounded-md transition-all flex-1 sm:flex-none text-center cursor-pointer', paymentStatusFilter === 'IPD' ? 'bg-white text-teal-600 shadow-sm' : 'text-slate-600 hover:text-slate-900']"
                >
                  IPD
                </button>
                <button 
                  @click="paymentStatusFilter = ''"
                  :class="['px-3 py-1.5 rounded-md transition-all flex-1 sm:flex-none text-center cursor-pointer', paymentStatusFilter === '' ? 'bg-white text-teal-600 shadow-sm' : 'text-slate-600 hover:text-slate-900']"
                >
                  All
                </button>
              </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
            <div>
              ...
              <input 
                ...
                type="date"
                class="w-full px-2.5 py-1.5 bg-white border border-slate-200 rounded-lg text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-teal-100 focus:border-teal-500"
              />
            </div>
            <div>
            ...
            <div class="flex items-end justify-between">
              <button 
                ...
                @click="clearFilters"
                class="px-2.5 py-1.5 text-xs text-slate-500 font-bold hover:text-teal-600 hover:bg-teal-50 rounded-lg transition-all cursor-pointer"
              >
                Reset Filters
              </button>
            </div>
          ...
          <div class="relative w-full">
            ...
            <input 
              ...
              class="w-full pl-10 pr-4 py-2 bg-white border border-slate-200 rounded-xl text-sm placeholder-slate-400 text-slate-700 focus:outline-none focus:ring-2 focus:ring-teal-100 focus:border-teal-500 transition-all shadow-sm"
            />
          </div>
        </div>
        ...
        <div class="flex-grow overflow-y-auto">
          <!-- Loading state -->
          <div v-if="endoscopyStore.loading && endoscopyStore.orders.length === 0" class="flex flex-col items-center justify-center h-full text-slate-400 py-12">
          ...
          <!-- Empty state -->
          <div v-else-if="endoscopyStore.orders.length === 0" class="flex flex-col items-center justify-center h-full text-slate-400 p-8 py-16">
          ...
          <table v-else class="w-full text-left text-xs whitespace-nowrap">
            ...
            <tbody class="divide-y divide-slate-100 text-slate-700">
              <tr 
                v-for="order in endoscopyStore.orders" 
                :key="order._id"
                @click="handleSelectOrder(order)"
                :class="['hover:bg-slate-50/80 cursor-pointer transition-all', selectedOrder && selectedOrder._id === order._id ? 'bg-teal-50/50 hover:bg-teal-50 font-medium' : '']"
              >
                <td class="px-6 py-4">
        ...
        <div v-if="totalPages > 1" class="p-4 border-t border-slate-100 flex items-center justify-between bg-slate-50/30">
          <button 
            ...
            class="px-3 py-1.5 text-xs font-semibold text-slate-700 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors cursor-pointer"
          >
            Previous
          </button>
      ...
      <div class="lg:col-span-5 h-[700px]">
        ...
        <div v-else class="bg-white border border-slate-100 rounded-2xl shadow-sm h-full flex flex-col justify-center items-center text-slate-400 p-8 text-center">
          ...
          <p class="text-slate-400 text-xs mt-1 max-w-[240px]">
            Please click on any endoscopy order from the list on the left to see details and process billing.
          </p>
        </div>
      </div>
    </div>
    ...
    <div v-if="showPaymentModal && activeBill" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div class="p-6 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
          ...
          <button @click="showPaymentModal = false" class="text-slate-400 hover:text-slate-600 rounded-lg p-1 transition-colors cursor-pointer">
        ...
        <div class="p-6 space-y-4">
          <!-- Totals Info -->
          <div class="bg-teal-50/40 border border-teal-100 p-4 rounded-xl grid grid-cols-2 gap-3 text-xs">
            <div>
            ...
            <div class="col-span-2 border-t border-teal-100 pt-2 flex justify-between items-center text-sm">
              <span class="font-semibold text-teal-950">Amount Payable:</span>
          ...
          <div class="space-y-4">
            ...
            <div class="space-y-1.5">
              ...
              <select 
                ...
                class="w-full px-3.5 py-2.5 bg-white border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-teal-100 focus:border-teal-500 text-slate-700 transition-all shadow-sm"
              >
            ...
            <div class="space-y-1.5">
              ...
              <input 
                ...
                class="w-full px-3.5 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-mono focus:outline-none focus:ring-2 focus:ring-teal-100 focus:border-teal-500 text-slate-700 transition-all shadow-sm"
              />
            </div>
            ...
            <div class="space-y-1.5">
              ...
              <textarea 
                ...
                rows="2"
                class="w-full px-3.5 py-2.5 bg-white border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-teal-100 focus:border-teal-500 text-slate-700 transition-all shadow-sm resize-none"
              ></textarea>
            </div>
          </div>
        </div>
        ...
        <div class="p-6 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-3">
          ...
          <button 
            ...
            class="px-5 py-2.5 text-sm font-bold text-white bg-emerald-600 hover:bg-emerald-700 rounded-xl shadow-lg shadow-emerald-100 transition-all flex items-center gap-2 disabled:opacity-50 cursor-pointer"
          >
