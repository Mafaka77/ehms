...
const fetchOrders = async () => {
  await endoscopyStore.fetchOrders(currentPage.value, limit.value, searchQuery.value, 'IPD', { admissionId: 'not-null' })
}

const updateOrderStatus = async (orderId, newStatus) => {
  try {
    const res = await endoscopyStore.updateOrder(orderId, { status: newStatus })
    if (res.success) {
      snackbarStore.show({
...
const getStatusColor = (status) => {
  const map = {
    ...
    'IPD': 'bg-teal-50 text-teal-700 border-teal-200'
  }
...
<template>
  <div class="space-y-6">
    ...
    <div class="bg-white rounded-2xl border border-slate-100 p-6 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm bg-slate-50/20">
      <div>
        <h2 class="text-lg font-bold text-slate-800">IPD Inpatient Orders</h2>
        <p class="text-slate-400 text-xs mt-0.5">Orders automatically billed to IPD patient accounts.</p>
      </div>
    ...
    <!-- Loading State -->
    <div v-if="endoscopyStore.loading" class="flex flex-col items-center justify-center py-24 text-slate-400 bg-white rounded-2xl border border-slate-100 shadow-sm">
      ...
      <span class="text-sm font-medium">Loading IPD orders...</span>
    </div>

    <!-- Empty State -->
    <div v-else-if="endoscopyStore.orders.length === 0" class="p-6 text-center text-slate-500 py-24 bg-white rounded-2xl border border-slate-100 shadow-sm">
      ...
      <p class="text-slate-700 font-semibold text-lg">No IPD endoscopy orders found</p>
      <p class="text-slate-400 text-sm mt-1 max-w-sm mx-auto">
        {{ searchQuery ? "No results match your search query." : "Admitted patient orders will appear here automatically." }}
      </p>
    </div>

    <!-- IPD Orders Cards Grid -->
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        ...
        >
          ...
          <div :class="[
            ...
            order.priority === 'URGENT' ? 'bg-amber-500' : 'bg-teal-500'
          ]"></div>
            ...
            <div class="flex items-center gap-3 w-full">
              <div class="flex-grow">
                ...
                <select 
                  ...
                  class="w-full text-xs font-semibold bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-slate-700 focus:outline-none focus:ring-2 focus:ring-teal-100 focus:border-teal-500 transition-all cursor-pointer"
                >
              ...
              <button 
                ...
                class="bg-teal-50 hover:bg-teal-100 text-teal-600 px-3.5 py-2 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 shadow-sm active:scale-95 cursor-pointer font-bold"
              >
                <span>Details</span>
      ...
      <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[85vh]">
        ...
        <div class="px-6 py-4.5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div>
            ...
            <p class="font-mono text-xs text-teal-600 font-semibold mt-0.5">{{ selectedOrder.orderNo }}</p>
          </div>
        ...
        <div class="p-6 overflow-y-auto space-y-6">
          ...
          <!-- Endoscopy Tests List -->
          <div>
            <h4 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Requested Endoscopy Tests</h4>
            <div class="border border-slate-100 rounded-xl overflow-hidden shadow-inner bg-white">
              <table class="w-full border-collapse text-left text-sm">
                <thead>
                  <tr class="bg-slate-50 border-b border-slate-100">
                    <th class="px-4 py-3 text-xs font-bold text-slate-500 uppercase">Procedure Name</th>
                ...
                <tbody class="divide-y divide-slate-100">
                  <tr v-for="item in selectedOrderItems" :key="item._id" class="hover:bg-slate-50/50">
                    <td class="px-4 py-3 text-slate-700 font-medium">{{ item.endoscopyTestId?.name }}</td>
        ...
        <div class="px-6 py-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between gap-4">
          <div class="flex items-center gap-2">
            ...
            <select 
              ...
              class="text-xs font-semibold bg-white border border-slate-200 rounded-xl px-3 py-1.5 text-slate-700 focus:outline-none focus:ring-2 focus:ring-teal-100 focus:border-teal-500 transition-all cursor-pointer shadow-sm"
            >
              <option value="ORDERED">Ordered</option>
              <option value="SCHEDULED">Scheduled</option>
              <option value="IN_PROGRESS">In Progress</option>
