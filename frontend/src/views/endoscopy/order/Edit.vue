  ...
  try {
    ...
    if (items.length > 0) {
      selectedTests.value = items.map(item => ({
        testId: item.endoscopyTestId?._id || item.endoscopyTestId,
        testName: item.endoscopyTestId?.name || 'Unknown Test',
        testCode: item.endoscopyTestId?.code || '',
        rate: item.rate,
        amount: item.amount
      }))
    }
  } catch (err) {
    error.value = 'Failed to load endoscopy order details'
...
<template>
  <div class="max-w-7xl mx-auto space-y-8">
    ...
    <div class="flex items-center gap-4">
      ...
      <div>
        <h1 class="text-2xl font-bold text-slate-900">Edit Endoscopy Order</h1>
        <p class="text-slate-500 mt-0.5 text-sm">Update the details of this endoscopy order.</p>
      </div>
    </div>

    <!-- Page Loading -->
    <div v-if="pageLoading" class="flex flex-col items-center justify-center py-32">
      <svg class="animate-spin h-10 w-10 text-teal-600 mb-4" fill="none" viewBox="0 0 24 24">
        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
      </svg>
      <p class="text-slate-400 text-sm font-medium">Loading endoscopy order details...</p>
    </div>

    <template v-else>
      <!-- Order Info Banner -->
      <div v-if="orderInfo" class="bg-gradient-to-r from-teal-50 to-emerald-50 border border-teal-100 rounded-2xl px-6 py-4 flex flex-wrap items-center gap-5">
        <div>
          <p class="text-xs text-slate-500 font-medium uppercase tracking-wider">Order No</p>
          <p class="text-base font-bold font-mono text-teal-700 mt-0.5">{{ orderInfo.orderNo }}</p>
        </div>
        <div class="w-px h-8 bg-teal-100 hidden sm:block"></div>
        <div>
        ...
        <div>
          ...
          <p class="text-sm font-bold text-teal-700 mt-0.5">{{ orderInfo.status?.replace('_', ' ') }}</p>
        </div>
      </div>
        ...
        <div class="lg:col-span-2 space-y-6">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class="relative">
                  ...
                  <div v-if="selectedPatient" class="flex items-center justify-between px-4 py-2.5 bg-teal-50 border border-teal-200 rounded-xl">
                    ...
                    <button type="button" @click="clearPatient" class="text-teal-400 hover:text-teal-600 cursor-pointer">
                ...
                <div v-if="form.referral === 'Doctor'" class="relative">
                  ...
                  <div v-if="selectedDoctor" class="flex items-center justify-between px-4 py-2.5 bg-teal-50 border border-teal-200 rounded-xl">
                    <div>
                      ...
                      <span class="text-xs text-teal-700 block">{{ selectedDoctor.doctorCode || selectedDoctor.specializationId?.name || '' }}</span>
                    </div>
                    <button type="button" @click="clearDoctor" class="text-teal-400 hover:text-teal-600 cursor-pointer">
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                    </button>
                  </div>
                  <div v-else>
                ...
                <div>
                  ...
                  <select v-model="form.priority" class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-teal-100 focus:border-teal-500 shadow-inner" :disabled="loading">
                ...
                <div>
                  ...
                  <select v-model="form.status" class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-teal-100 focus:border-teal-500 shadow-inner" :disabled="loading">
                ...
                <div>
                  ...
                  <select v-model="form.paymentStatus" class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-teal-100 focus:border-teal-500 shadow-inner" :disabled="loading">
          ...
          <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden flex flex-col h-96">
            <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/50 shrink-0 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
              <h2 class="text-base font-semibold text-slate-800">Endoscopy Tests</h2>
        ...
        <div class="lg:col-span-1">
          <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden sticky top-8">
            ...
            <div class="p-6">
              ...
              <button
                type="button"
                @click="router.push({ name: 'endoscopy-order' })"
                class="w-full mt-3 px-5 py-2.5 rounded-xl border border-slate-200 text-slate-600 text-sm font-medium hover:bg-slate-50 transition-colors cursor-pointer"
              >
                Cancel
              </button>
            </div>
      ...
      <div v-if="showPatientModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-md relative z-10 overflow-hidden">
          <div class="px-6 py-4 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
            ...
            <button @click="showPatientModal = false" class="text-slate-400 hover:text-slate-600 cursor-pointer">
