...
const fetchOrders = async () => {
  await endoscopyStore.fetchOrders(currentPage.value, limit.value, searchQuery.value, paymentFilter.value)
}
...
const getPaymentColor = (status) => {
  const map = {
    ...
    IPD: 'bg-teal-50 text-teal-600 border-teal-200'
  }
...
const handleDelete = async (id) => {
  if (!confirm('Delete this endoscopy order? All associated items will also be deleted.')) return
  const res = await endoscopyStore.deleteOrder(id)
  if (res.success) {
    snackbarStore.show({ message: 'Endoscopy order deleted successfully', type: 'success' })
    if (endoscopyStore.orders.length === 0 && currentPage.value > 1) {
      currentPage.value--
    } else {
      fetchOrders()
    }
...
<template>
  <div class="space-y-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        <div class="flex items-center gap-3 mb-1">
          ...
          <h1 class="text-2xl font-bold text-slate-900">Endoscopy Orders</h1>
        </div>
        <p class="text-slate-500 text-sm">Manage and track patient endoscopy orders.</p>
      </div>
      <button
        @click="router.push({ name: 'endoscopy-order-create' })"
        class="bg-teal-600 hover:bg-teal-700 text-white px-5 py-2.5 rounded-xl font-medium text-sm shadow-lg shadow-teal-100 hover:shadow-teal-200 transition-all active:scale-95 flex items-center gap-2 self-start sm:self-auto cursor-pointer"
      >
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <div class="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
          ...
          <div class="relative w-full sm:w-72">
            ...
            <input
              ...
              class="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-teal-100 focus:border-teal-500 transition-all"
            />
          </div>
          <!-- Payment filter -->
          <select
            v-model="paymentFilter"
            class="px-3 py-2.5 bg-white border border-slate-200 rounded-xl text-sm text-slate-600 focus:outline-none focus:ring-2 focus:ring-teal-100 focus:border-teal-500 transition-all"
          >
      ...
      <!-- Loading -->
      <div v-if="endoscopyStore.loading" class="flex flex-col items-center justify-center py-24">
        ...
        <span class="text-sm text-slate-400 font-medium">Loading endoscopy orders...</span>
      </div>

      <!-- Empty -->
      <div v-else-if="endoscopyStore.orders.length === 0" class="flex flex-col items-center justify-center py-24 text-slate-400">
        ...
        <p class="text-slate-700 font-semibold text-lg">No endoscopy orders found</p>
        <p class="text-slate-400 text-sm mt-1">
          {{ searchQuery || paymentFilter ? 'No results match your filters.' : "Click 'Create Order' to register a new endoscopy order." }}
        </p>
      </div>
            ...
            >
              ...
              <td class="px-6 py-4 text-center">
                <div class="flex items-center justify-center gap-2">
                  <button
                    @click="router.push({ name: 'endoscopy-order-edit', params: { id: order._id } })"
                    class="p-2 rounded-xl text-slate-400 hover:bg-teal-50 hover:text-teal-600 transition-all focus:outline-none focus:ring-2 focus:ring-teal-100 cursor-pointer"
                    title="Edit Order"
                  >
                  ...
                  <button
                    ...
                    class="p-2 rounded-xl text-slate-400 hover:bg-rose-50 hover:text-rose-600 transition-all focus:outline-none focus:ring-2 focus:ring-rose-100 cursor-pointer"
                    title="Delete Order"
                  >
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
      ...
      <!-- Pagination -->
      <div
        v-if="endoscopyStore.orderPagination.total > 0"
      ...
      >
        <span class="text-sm text-slate-500 font-medium">
          ...
          to
          <span class="text-slate-800 font-semibold">{{ Math.min(currentPage * limit, endoscopyStore.orderPagination.total) }}</span>
          of
          <span class="text-slate-800 font-semibold">{{ endoscopyStore.orderPagination.total }}</span>
          entries
        </span>
        <div v-if="endoscopyStore.orderPagination.pages > 1" class="flex items-center gap-2">
          <button
          ...
          <button
            v-for="page in endoscopyStore.orderPagination.pages"
            :key="page"
            @click="currentPage = page"
            class="w-9 h-9 rounded-xl flex items-center justify-center text-sm font-medium transition-all"
            :class="currentPage === page ? 'bg-teal-600 text-white shadow-lg shadow-teal-100' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'"
          >{{ page }}</button>
          <button
            @click="currentPage < endoscopyStore.orderPagination.pages && currentPage++"
            :disabled="currentPage === endoscopyStore.orderPagination.pages"
            class="p-2 rounded-xl border border-slate-200 bg-white text-slate-600 hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
          >
