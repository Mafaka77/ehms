...
import RadiologyInvoiceModal from './Invoice.vue'
...
const fetchBillDetails = async (billId) => {
  ...
  try {
    const data = await radiologyStore.fetchBillDetails(billId)
...
const generateBill = async () => {
  ...
  try {
    const payload = {
      radiologyOrderId: props.order._id,
    ...
    const data = await radiologyStore.generateBill(payload)
    snackbarStore.show({
...
const cancelBill = async () => {
  ...
  try {
    await radiologyStore.cancelBill(billDetails.value._id)
    snackbarStore.show({
...
const handleRevertPayment = async (payment) => {
  ...
  try {
    await radiologyStore.cancelPayment(payment._id)
    snackbarStore.show({
...
<template>
  <div>
    ...
    <div class="p-6 flex-grow overflow-y-auto space-y-6">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-100">
        <div>
          ...
          <p class="font-bold text-slate-800 mt-1 flex items-center gap-1.5 flex-wrap">
            ...
            <span v-if="order.patientId?.isEmployee" class="px-1.5 py-0.5 text-[10px] font-bold rounded bg-violet-100 text-violet-700 border border-violet-200">
      ...
      <!-- Test list -->
      <div>
        <h3 class="text-sm font-bold text-slate-800 uppercase tracking-wider mb-3">Ordered Radiology Tests</h3>
          ...
          <table class="w-full text-left text-xs">
            ...
            <tbody class="divide-y divide-slate-100 text-slate-700">
              <tr v-for="item in order.items" :key="item._id" class="hover:bg-slate-50/50">
                <td class="px-4 py-3 font-medium text-slate-800">{{ item.radiologyTestId?.name || 'Radiology Test' }}</td>
              ...
              <tr class="bg-slate-50/50 font-bold border-t border-slate-100">
                ...
                <td class="px-4 py-3 text-right font-mono text-violet-600 text-sm">{{ formatCurrency(order.totalAmount) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div v-if="!order.billId && order.paymentStatus === 'UNPAID'" class="bg-slate-50 border border-slate-200/60 rounded-xl p-4 space-y-3">
        <div class="flex items-center justify-between">
          <label class="flex items-center gap-2 cursor-pointer text-xs font-bold text-slate-700 uppercase select-none">
            <input type="checkbox" v-model="showDiscount" class="text-violet-600 focus:ring-violet-500 rounded border-slate-300">
            Apply Discount
          </label>
        ...
        <div v-if="showDiscount" class="grid grid-cols-2 gap-3 pt-2 border-t border-slate-200/50">
          <div class="col-span-2 sm:col-span-1">
            ...
            <select 
              ...
              class="w-full px-2.5 py-1.5 bg-white border border-slate-200 rounded-lg text-xs focus:outline-none focus:ring-1 focus:ring-violet-500 text-slate-700"
            >
          ...
          <div v-if="discountMode !== 'employee'" class="col-span-2 sm:col-span-1">
            ...
            <input 
              ...
              type="number"
              min="0"
              class="w-full px-2.5 py-1.5 bg-white border border-slate-200 rounded-lg text-xs focus:outline-none focus:ring-1 focus:ring-violet-500 text-slate-700 font-mono"
            />
          </div>
          ...
          <div v-else class="col-span-2 sm:col-span-1 relative">
            ...
            <div v-if="selectedEmployee" class="flex items-center justify-between px-2.5 py-1.5 bg-violet-50 border border-violet-200 rounded-lg">
              <div class="flex flex-col overflow-hidden w-full">
                <span class="text-xs font-bold text-violet-900 truncate">{{ selectedEmployee.fullName }}</span>
                <span class="text-[9px] text-violet-700 font-mono truncate">{{ selectedEmployee.employeeCode }}</span>
              </div>
              <button type="button" @click="clearEmployee" class="text-violet-400 hover:text-violet-600 focus:outline-none ml-2">
          ...
          <div class="col-span-2">
            ...
            <input 
              ...
              class="w-full px-2.5 py-1.5 bg-white border border-slate-200 rounded-lg text-xs focus:outline-none focus:ring-1 focus:ring-violet-500 text-slate-700 placeholder-slate-400"
            />
          </div>
        </div>
        ...
        <div v-if="showDiscount" class="pt-2.5 border-t border-dashed border-slate-200 space-y-1.5 text-xs">
          ...
          <div class="flex justify-between items-center text-slate-800 font-bold border-t border-slate-200/50 pt-1.5">
            ...
            <span class="font-mono text-violet-600">{{ formatCurrency(netAmount) }}</span>
          </div>
        </div>
      </div>
      ...
      <div v-if="billDetails" class="space-y-3 bg-violet-50/40 p-4 rounded-xl border border-violet-100/50">
        <div class="flex justify-between items-center border-b border-violet-100/50 pb-2">
          <div>
            <h4 class="text-xs font-bold uppercase text-violet-500">Associated Bill Info</h4>
            <span class="font-mono font-bold text-xs text-violet-950 mt-0.5">{{ billDetails.billNo }}</span>
          </div>
        ...
        <div class="grid grid-cols-2 gap-3 text-xs">
          ...
          <div>
            ...
            <p class="font-bold text-violet-900 font-mono">{{ formatCurrency(billDetails.netAmount) }}</p>
          </div>
          <div>
    ...
    <div class="p-6 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-3">
      ...
      <button
        ...
        class="w-full bg-violet-600 hover:bg-violet-700 text-white py-3 px-5 rounded-xl font-semibold text-sm shadow-lg shadow-violet-100 hover:shadow-violet-200 transition-all transform active:scale-95 flex items-center justify-center gap-2 disabled:opacity-50"
      >
      ...
      <div v-else-if="billDetails && billDetails.status === 'PAID'" class="w-full flex flex-col gap-2.5">
        ...
        <button
          @click="printBill"
          type="button"
          class="w-full bg-violet-600 hover:bg-violet-700 text-white py-3 px-5 rounded-xl font-semibold text-sm shadow-lg shadow-violet-100 hover:shadow-violet-200 transition-all transform active:scale-95 flex items-center justify-center gap-2"
        >
