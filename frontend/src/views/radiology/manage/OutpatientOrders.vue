...
const fetchOrders = async () => {
  await radiologyStore.fetchOrders(currentPage.value, limit.value, searchQuery.value, 'PAID', { admissionId: 'null' })
}
...
const viewDetails = async (order) => {
  ...
  selectedPerformedById.value = order.performedById?._id || order.performedById || ''
  ...
  try {
    ...
    if (res.order) {
      ...
      selectedPerformedById.value = res.order.performedById?._id || res.order.performedById || ''
    }
  } catch (error) {
...
const updatePerformedDoctor = async () => {
  ...
  try {
    const res = await radiologyStore.updateOrder(selectedOrder.value._id, {
      performedById: selectedPerformedById.value || null
    })
    if (res.success) {
      snackbarStore.show({ message: 'Performing doctor updated successfully!', type: 'success' })
      if (res.data) {
        selectedOrder.value.performedById = res.data.performedById
      }
      await fetchOrders()
    } else {
    ...
    snackbarStore.show({ message: 'An error occurred while updating performing doctor', type: 'error' })
  }
}
  ...
  <div class="space-y-6">
    ...
    <div class="bg-white rounded-2xl border border-slate-100 p-6 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm bg-slate-50/20">
      <div>
        ...
        <p class="text-slate-400 text-xs mt-0.5">Orders ready for radiology imaging scans.</p>
      </div>
    ...
    <div v-else-if="radiologyStore.orders.length === 0" class="p-6 text-center text-slate-500 py-24 bg-white rounded-2xl border border-slate-100 shadow-sm">
      ...
      <p class="text-slate-400 text-sm mt-1 max-w-sm mx-auto">
        {{ searchQuery ? "No results match your search query." : "Radiology orders appear here once they are paid." }}
      </p>
    </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        ...
        >
          ...
          <div class="space-y-2.5 border-t border-b border-slate-100/80 py-4 my-3 text-sm">
            ...
            <div v-if="order.performedById" class="flex items-center justify-between">
              <span class="text-slate-400 text-xs">Performed By</span>
              <span class="text-indigo-600 font-semibold text-right truncate max-w-[180px]">
                {{ order.performedById?.fullName }}
              </span>
            </div>
          </div>
