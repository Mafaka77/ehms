...
const handleSubmit = async () => {
  ...
  try {
    if (props.category) {
      const response = await radiologyStore.updateCategory(props.category._id, form)
      if (response.success) {
      ...
      const response = await radiologyStore.createCategory(form)
      if (response.success) {
      ...
      <div v-if="show" class="fixed inset-0 z-[999] flex items-center justify-center p-4 overflow-x-hidden overflow-y-auto">
        ...
        <div class="bg-white rounded-2xl w-full max-w-lg shadow-2xl border border-slate-200 overflow-hidden transform transition-all relative z-10">
          ...
          <div class="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
            <div>
              <h3 class="text-lg font-bold text-slate-900">{{ category ? 'Edit Radiology Category' : 'Add New Radiology Category' }}</h3>
              <p class="text-xs text-slate-500 mt-0.5">{{ category ? 'Modify the details of this category.' : 'Define a new radiology category (e.g. CT Scan, MRI).' }}</p>
            </div>
            <button 
              @click="handleClose" 
          ...
          <form @submit.prevent="handleSubmit" class="p-6 space-y-5">
            ...
            <BaseInput 
              ...
              id="name"
              label="Category Name"
              placeholder="e.g. CT Scan, Ultrasound"
              required
              :disabled="loading"
            />
            ...
            <div>
              ...
              <textarea
                ...
                :disabled="loading"
                rows="3"
                placeholder="Brief description of this radiology category..."
                class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 transition-all resize-none disabled:bg-slate-50 disabled:text-slate-400"
              ></textarea>
            </div>
            ...
            <div class="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100">
              ...
              <div>
                ...
                <p class="text-xs text-slate-400 mt-0.5">Inactive categories won't be visible when creating orders</p>
              </div>
            </div>
