...
const handleSubmit = async () => {
  ...
  try {
    ...
    const response = await radiologyStore.createOrder(orderData)
    if (response.success) {
...
<template>
  <div class="max-w-7xl mx-auto space-y-8">
    ...
    <div class="flex items-center gap-4">
      ...
      <div>
        <h1 class="text-2xl font-bold text-slate-900">Create Radiology Order</h1>
        <p class="text-slate-500 mt-0.5 text-sm">Fill in the details to register a new radiology order.</p>
      </div>
    </div>
      ...
      <div class="lg:col-span-2 space-y-6">
          ...
          <div class="p-6 space-y-5">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div class="relative">
                ...
                <div v-if="selectedOpdAppointment" class="flex items-center justify-between px-4 py-2.5 bg-indigo-50 border border-indigo-200 rounded-xl">
                  <div class="flex flex-col">
                    ...
                    <span class="text-xs text-indigo-700" v-if="selectedOpdAppointment.patientId">{{ selectedOpdAppointment.patientId.fullName }}</span>
                  </div>
                  <button type="button" @click="clearOpdAppointment" class="text-indigo-400 hover:text-indigo-600">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                  </button>
                </div>
                <div v-else>
                  <div class="relative flex items-center border border-slate-200 rounded-xl bg-white focus-within:ring-2 focus-within:ring-indigo-100 focus-within:border-indigo-500 overflow-hidden">
              ...
              <div v-if="form.referral === 'Doctor'" class="relative">
                ...
                <div v-if="selectedDoctor" class="flex items-center justify-between px-4 py-2.5 bg-indigo-50 border border-indigo-200 rounded-xl">
                  <div>
                    ...
                    <span class="text-xs text-indigo-700 block">{{ selectedDoctor.doctorCode }}</span>
                  </div>
              ...
              <div>
                ...
                <select v-model="form.priority" class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 transition-all shadow-inner" :disabled="loading">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden flex flex-col h-96">
          <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/50 shrink-0 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            <h2 class="text-base font-semibold text-slate-800">Add Radiology Tests</h2>
