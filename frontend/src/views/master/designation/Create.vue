...
watch(() => props.show, (newVal) => {
  if (newVal) {
    if (props.designation) {
      form.designationName = props.designation.designationName || ''
      form.isActive = props.designation.isActive !== false
    } else {
...
const handleSubmit = async () => {
  if (!form.designationName.trim()) {
    error.value = 'Designation name is required'
    return
  }
  ...
  try {
    if (props.designation) {
      // Edit Mode
      const response = await masterDesignationStore.updateDesignation(props.designation._id, form)
      if (response.success) {
        snackbarStore.show({
          message: response.message || 'Designation updated successfully!',
          type: 'success'
        })
        emit('updated', response.data)
        handleClose()
      } else {
        error.value = response.message || 'Failed to update designation'
        snackbarStore.show({
          message: response.message || 'Failed to update designation',
          type: 'error'
        })
      }
    } else {
      // Create Mode
      const response = await masterDesignationStore.createDesignation(form)
      if (response.success) {
        snackbarStore.show({
        ...
        error.value = response.message || 'Failed to create designation'
        snackbarStore.show({
          message: response.message || 'Failed to create designation',
          type: 'error'
        })
      }
    }
      ...
      <div v-if="show" class="fixed inset-0 z-[999] flex items-center justify-center p-4 overflow-x-hidden overflow-y-auto">
        ...
        <div class="bg-white rounded-2xl w-full max-w-lg shadow-2xl border border-slate-200 overflow-hidden transform transition-all relative z-10">
          ...
          <div class="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
            <div>
              <h3 class="text-lg font-bold text-slate-900">{{ designation ? 'Edit Designation' : 'Add New Designation' }}</h3>
              <p class="text-xs text-slate-500 mt-0.5">{{ designation ? 'Modify the details of this designation.' : 'Define a new job role or medical staff title.' }}</p>
            </div>
            <button 
              @click="handleClose" 
          ...
          <form @submit.prevent="handleSubmit" class="p-6 space-y-5">
            ...
            <BaseInput 
              ...
              placeholder="e.g. Senior Registrar, Staff Nurse, Head Accountant"
              required
              :disabled="loading"
            />
            ...
            <div class="flex items-center gap-3 pl-1">
              <input 
                ...
                :disabled="loading"
                class="w-4 h-4 text-indigo-600 border-slate-300 rounded focus:ring-indigo-500 focus:ring-offset-0"
              />
            ...
            <div class="flex justify-end items-center gap-3 pt-3 border-t border-slate-100">
              ...
              >
                ...
                <span>{{ loading ? 'Saving...' : (designation ? 'Save Changes' : 'Save Designation') }}</span>
              </button>
            </div>
          </form>
