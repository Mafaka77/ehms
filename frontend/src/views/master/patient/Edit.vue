...
watch(() => props.patient, (newVal) => {
  if (newVal) {
    form.value = {
      ...
      fathersName: newVal.fathersName || '',
      mothersName: newVal.mothersName || '',
      maritalStatus: newVal.maritalStatus || '',
      religion: newVal.religion || '',
      husbandwifeName: newVal.husbandwifeName || '',
      gender: newVal.gender || 'Male',
      dateOfBirth: formatDateForInput(newVal.dateOfBirth),
      age: newVal.age ?? null,
      bloodGroup: newVal.bloodGroup || '',
      occupation: newVal.occupation || '',
      mobileNo: newVal.mobileNo || '',
      alternateMobileNo: newVal.alternateMobileNo || '',
      email: newVal.email || '',
      address: newVal.address || '',
      contactPerson: newVal.contactPerson || '',
      contactPersonRelation: newVal.contactPersonRelation || '',
      contactPersonMobile: newVal.contactPersonMobile || '',
      contactPersonAddress: newVal.contactPersonAddress || '',
      allergies: Array.isArray(newVal.allergies) ? newVal.allergies.join(', ') : (newVal.allergies || ''),
...
const handleUpdate = async () => {
  ...
  const res = await patientStore.updatePatient(props.patient._id, payload)
  loading.value = false

  if (res.success) {
    snackbarStore.show({ message: 'Patient details updated successfully!', type: 'success' })
    emit('updated', res.data)
    emit('close')
  } else {
    snackbarStore.show({ message: res.message || 'Failed to update patient', type: 'error' })
  }
}
</script>
    ...
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-150">
      ...
      <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
        <div>
          <div class="flex items-center gap-2">
            <h3 class="text-base font-bold text-slate-800">Edit Patient Details</h3>
            <span v-if="props.patient?.patientCode" class="text-xs font-bold text-indigo-600 bg-indigo-50 border border-indigo-100 px-2 py-0.5 rounded-md">
              {{ props.patient.patientCode }}
            </span>
          </div>
          <p class="text-xs text-slate-400 mt-0.5">Update demographic and medical contact details</p>
        </div>
        <button
          @click="emit('close')"
      ...
      <div class="px-6 py-3.5 border-t border-slate-100 bg-slate-50/50 flex items-center justify-end gap-2">
        ...
        >
          ...
          <span>{{ loading ? 'Saving...' : 'Update Patient' }}</span>
        </button>
      </div>
