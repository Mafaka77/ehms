...
const handleSubmit = async () => {
  ...
  try {
    if (props.department) {
      // Edit Mode
      const response = await masterDepartmentStore.updateDepartment(props.department._id, form)
      if (response.success) {
        snackbarStore.show({
        ...
        error.value = response.message || 'Failed to update department'
        snackbarStore.show({
          message: response.message || 'Failed to update department',
          type: 'error'
        })
      }
    } else {
      // Create Mode
      const response = await masterDepartmentStore.createDepartment(form)
      if (response.success) {
        snackbarStore.show({
        ...
        error.value = response.message || 'Failed to create department'
        snackbarStore.show({
          message: response.message || 'Failed to create department',
          type: 'error'
        })
      }
    }
      ...
      <div v-if="show" class="fixed inset-0 z-[999] flex items-center justify-center p-4 overflow-x-hidden overflow-y-auto">
        ...
        <div class="bg-white rounded-2xl w-full max-w-lg shadow-2xl border border-slate-200 overflow-hidden transform transition-all relative z-10">
          ...
          <div class="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
            <div>
              <h3 class="text-lg font-bold text-slate-900">{{ department ? 'Edit Department' : 'Add New Department' }}</h3>
              <p class="text-xs text-slate-500 mt-0.5">{{ department ? 'Modify the details of this department.' : 'Define a new medical unit or operational division.' }}</p>
            </div>
            <button 
              @click="handleClose" 
          ...
          <form @submit.prevent="handleSubmit" class="p-6 space-y-5">
            ...
            <BaseInput 
              ...
              placeholder="Administration,Laboratory,Nursing etc"
              required
              :disabled="loading"
            />
            ...
            <BaseTextarea 
              ...
              id="description"
              label="Description"
              placeholder="Describe the department responsibilities or services offered..."
              :disabled="loading"
            />
            ...
            <div class="flex justify-end items-center gap-3 pt-3 border-t border-slate-100">
              ...
              >
                ...
                <span>{{ loading ? 'Saving...' : (department ? 'Save Changes' : 'Save Department') }}</span>
              </button>
            </div>
          </form>
