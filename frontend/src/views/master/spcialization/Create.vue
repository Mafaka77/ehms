...
watch(() => props.show, (newVal) => {
  if (newVal) {
    if (props.specialization) {
      form.code = props.specialization.code || ''
      form.name = props.specialization.name || ''
      form.description = props.specialization.description || ''
    } else {
      form.code = ''
      form.name = ''
      form.description = ''
...
const handleSubmit = async () => {
  if (!form.code || !form.code.trim()) {
    error.value = 'Specialization code is required'
    return
  }
  if (!form.name || !form.name.trim()) {
    error.value = 'Specialization name is required'
    return
  }
  ...
  try {
    if (props.specialization) {
      const response = await masterSpecializationStore.updateSpecialization(props.specialization._id, form)
      if (response.success) {
        snackbarStore.show({
          message: response.message || 'Specialization updated successfully!',
          type: 'success'
        })
        emit('updated', response.data)
        handleClose()
      } else {
        error.value = response.message || 'Failed to update specialization'
        snackbarStore.show({
          message: response.message || 'Failed to update specialization',
          type: 'error'
        })
      }
    } else {
      const response = await masterSpecializationStore.createSpecialization(form)
      if (response.success) {
        snackbarStore.show({
          message: response.message || 'Specialization created successfully!',
          type: 'success'
        })
        emit('created', response.data)
        handleClose()
      } else {
        error.value = response.message || 'Failed to create specialization'
        snackbarStore.show({
          message: response.message || 'Failed to create specialization',
          type: 'error'
        })
      }
    }
      ...
      <div v-if="show" class="fixed inset-0 z-[999] flex items-center justify-center p-4 overflow-x-hidden overflow-y-auto">
        ...
        <div class="bg-white rounded-2xl w-full max-w-lg shadow-2xl border border-slate-200 overflow-hidden transform transition-all relative z-10">
          ...
          <div class="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
            <div>
              <h3 class="text-lg font-bold text-slate-900">{{ specialization ? 'Edit Specialization' : 'Add New Specialization' }}</h3>
              <p class="text-xs text-slate-500 mt-0.5">{{ specialization ? 'Modify the details of this specialization.' : 'Define a new medical specialization.' }}</p>
            </div>
            <button 
              @click="handleClose" 
          ...
          <form @submit.prevent="handleSubmit" class="p-6 space-y-5">
            ...
            <BaseTextarea 
              ...
              id="description"
              label="Description"
              placeholder="Describe the medical specialization..."
              :disabled="loading"
            />
            ...
            <div class="flex justify-end items-center gap-3 pt-3 border-t border-slate-100">
              ...
              >
                ...
                <span>{{ loading ? 'Saving...' : (specialization ? 'Save Changes' : 'Save Specialization') }}</span>
              </button>
            </div>
          </form>
