...
const fetchSupplierDetails = async () => {
  ...
  error.value = ''
  try {
    const res = await pharmacyStore.fetchSupplierById(props.id)
    if (res.success && res.data) {
      ...
      error.value = res.message || 'Failed to fetch supplier details'
      snackbarStore.show({ message: error.value, type: 'error' })
    }
  } catch (err) {
    console.error(err)
    error.value = err.response?.data?.message || err.message || 'An error occurred while loading supplier details'
    snackbarStore.show({ message: error.value, type: 'error' })
  } finally {
    loading.value = false
  }
...
const handleSubmit = async () => {
  ...
  try {
    const res = await pharmacyStore.updateSupplier(props.id, form.value)
    if (res.success) {
      snackbarStore.show({ message: res.message || 'Supplier updated successfully', type: 'success' })
      handleBack()
    } else {
      error.value = res.message || 'Failed to update supplier'
      snackbarStore.show({ message: error.value, type: 'error' })
    }
  } catch (err) {
    console.error(err)
  ...
  <div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
      ...
      <div>
        ...
        <p class="text-slate-500 text-sm mt-0.5">
          Modify the supplier details including contact, address, and billing information.
        </p>
      </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
      ...
      <div v-if="loading" class="flex flex-col items-center justify-center py-32 text-slate-400">
        ...
        <span class="text-sm font-medium">Loading supplier details...</span>
      </div>
